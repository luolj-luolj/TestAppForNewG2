...
void SensorEventProducer<AccelerometerData>::WorkFn() {
  ...
  if (!sensor.Start()) {
    PHONEAR_LOGE("accelerometer not started ~~~~~");
    return;
  }
  ...
  // this.
  PHONEAR_LOGI("accelerometer run_thread is %d", (event_producer_->run_thread)?1:0);
...
void SensorEventProducer<GyroscopeData>::WorkFn() {
  ...
  if (!sensor.Start()) {
    PHONEAR_LOGE("gyroscope not started ~~~~~");
    return;
  }
  ...
  // TODO(b/135468657): Investigate clock conversion. Old cardboard doesn't have
  // this.
  PHONEAR_LOGI("gyroscope run_thread is %d", (event_producer_->run_thread)?1:0);
  while (event_producer_->run_thread) {
    sensor.PollForSensorData(kMaxWaitMilliseconds, &sensor_events_vec);
